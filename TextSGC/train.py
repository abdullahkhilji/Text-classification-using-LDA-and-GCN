...
def eval_linear(model, features, lda_features, label, binary=False):
    model.eval()
    if not binary:
        act = partial(F.log_softmax, dim=1)
        criterion = F.nll_loss
...
if __name__ == '__main__':
    ...
    train_res = eval_linear(best_model, feat_dict["train"].cuda(), torch.from_numpy(np.asarray(lda_dict["train"])).cuda(),
